...
describe GoodPack do
  ...
  describe "#tax returns amount of tax for the good:" do
    it "10% for item" do
      good = GoodPack.new 1, "item", 10.0, tax_registry: FakeTaxRegistry
      expect(good.tax).to eq 1
    end

    it "double for 2 items" do
      good = GoodPack.new 2, "item", 10, tax_registry: FakeTaxRegistry
      expect(good.tax).to eq 2
    end

    it "rounded up to the nearest 0.05" do
      good = GoodPack.new 1, "item", 47.30, tax_registry: FakeTaxRegistry
  ...
  describe "#total returns price + tax, multiplied by quantity" do
    ...
    it "two items" do
      good = GoodPack.new 2, "item", 10.0, tax_registry: FakeTaxRegistry
